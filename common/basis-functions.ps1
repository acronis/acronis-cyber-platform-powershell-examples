...
function Update-Token {
  ...
  # A customer token expired as well, remove it
  if (Test-Path -Path "${scriptDir}\..\api_token_customer_scope.json" -PathType Leaf) {
      Remove-Item -Path "${scriptDir}\..\api_token_customer_scope.json" -Force
      Write-Host "A customer scoped token was deleted. Please re-issue another one manually if you need it."
  }
...
function Acronis-Delete {
  ...
  return Invoke-RestMethod -Method Delete -Uri "${baseUrl}${Uri}" -Headers $headers -Body $Body
