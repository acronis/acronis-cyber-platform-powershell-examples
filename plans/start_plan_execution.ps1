...
if ($null -ne $allMachines.items){
	...
	$appliedPolicies = Acronis-Get -Uri "/api/policy_management/v4/applications?context_id=${firstMachineResourceId}&policy_type=policy.backup.machine"
	...
	if ($null -ne $appliedPolicies.items -And $appliedPolicies.items.Length -gt 0){
		...
		{
			"state": "running",
			"policy_id": "${appliedPoliciesFirstPolicyId}",
			"context_ids": ["${firstMachineResourceId}"]
		}
"@

		# Run the first applicable policy for the first found machines
		# FOR DEMO PURPOSES ONLY
		Acronis-Put -Uri "/api/policy_management/v4/applications/run" -Body $json
