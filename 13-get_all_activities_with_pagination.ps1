...
# $page = Invoke-RestMethod -Uri "${baseUrl}api/task_manager/v2/activities?limit=${pageSize}" -Headers $headers
$page = Acronis-Get -Uri "api/task_manager/v2/activities?limit=${pageSize}"
...
while ($after) {
	...
	# $page = Invoke-RestMethod -Uri "${baseUrl}api/task_manager/v2/activities?limit=${pageSize}&after=${after}" -Headers $headers
	$page = Acronis-Get -Uri "api/task_manager/v2/activities" -Body $pagingParams
...
Write-Output "The activities were paged to the end."